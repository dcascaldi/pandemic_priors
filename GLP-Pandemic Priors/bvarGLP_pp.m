function r = bvarGLP_pp(y,lags,covid_ind,covid_periods,varargin)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function estimates the GLP-Pandemic Priors BVAR
...
% covid_ind:     index of when the pandemic starts in the data
% covid_periods: how many periods of the pandemic should be treated
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
numarg = nargin; % Necessary for setpriors_pp.m
setpriors_pp;
...
b(2:n+1,:)=diag(diagb);
b= [b;zeros(covid_periods,n)]; % Add the prior on the Pandemic dummies
...
inphi=-log((MAX.phi-phi0)/(phi0-MIN.phi));

x0=[-log((MAX.lambda-lambda0)/(lambda0-MIN.lambda));inpsi;intheta;inmiu;inalpha;inphi];

H0=10*eye(length(x0));          % initial guess for the inverse Hessian

%% maximization of the posterior of the hyperparameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% test_val = logMLVAR_formin_pp(x0,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef,covid_periods)

[~,xh,~,H,itct,~,~] = csminwel('logMLVAR_formin_pp',x0,H0,[],1e-16,1000,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef,covid_periods);

%% output of the maximization
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% VAR coefficients and residuals at the posterior model
[fh,r.postmax.betahat,r.postmax.sigmahat]=logMLVAR_formin_pp(xh,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef,covid_periods);
...
r.postmax.phi=MIN.phi+(MAX.phi-MIN.phi)/(1+exp(-xh(end)));
...
elseif mn.alpha==1
    ...
    r.postmax.alpha=MIN.alpha+(MAX.alpha-MIN.alpha)/(1+exp(-xh(end-1)));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if Fcast==1
    Y=[y;zeros(size(hz,2),n)];
    for tau=1:max(hz)
        xT=[[1;reshape(Y(T+tau-1:-1:T+tau-lags,:)',k-1,1)]',zeros(1,covid_periods)];
...
if mcmc==1
    ...
    if mn.alpha==1
        JJ(end-1)=(MAX.alpha-MIN.alpha)*JJ(end-1);
    end

    JJ(end)=(MAX.phi-MIN.phi)*JJ(end);
    ...
    modephi = r.postmax.phi;
    
    postmode=[r.postmax.lambda;modepsi;modetheta;modemiu;modealpha;modephi];
    ...
    while logMLold==-10e15
        ...
        [logMLold,~,~]=logMLVAR_formcmc_pp(P(1,:)',y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,...
            max([MCMCfcast,MCMCstorecoeff]),hyperpriors,priorcoef,covid_periods);
    end
    
    % matrix to store the draws of the VAR coefficients if MCMCstorecoeff is on
    if MCMCstorecoeff==1
        r.mcmc.beta  = zeros(k+covid_periods,n,M-N);
    ...
    for i=2:M
        ...
        [logMLnew,betadrawnew,sigmadrawnew]=logMLVAR_formcmc_pp(P(i,:)',y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,...
            max([MCMCfcast,MCMCstorecoeff]),hyperpriors,priorcoef,covid_periods);
        ...
        if logMLnew>logMLold
            ...
            if rand(1)<exp(logMLnew-logMLold)
                ...
                if MCMCfcast==1 || MCMCstorecoeff==1
                    [~,betadrawnew,sigmadrawnew]=logMLVAR_formcmc_pp(P(i,:)',y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,...
                        max([MCMCfcast,MCMCstorecoeff]),hyperpriors,priorcoef,covid_periods);
                end
            end
        end
        ...
        if i>N && MCMCfcast==1
            ...
            for tau=1:max(hz)
                ...
                Y(T+tau,:)=xT*betadrawnew(1:n*lags+1,:)+mvnrnd(zeros(1,n),sigmadrawnew);
            end
    ...
    if mn.alpha==1
        % Lag-decaying parameter of the MN prior
        r.mcmc.alpha=P(N+1:end,end-1);
    end
    
    r.mcmc.phi=P(N+1:end,end);
    % acceptance rate
