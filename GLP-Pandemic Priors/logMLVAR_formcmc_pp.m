function [logML,betadraw,drawSIGMA]=logMLVAR_formcmc_pp(par,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,draw,hyperpriors,priorcoef,covid_periods)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function computes the log-posterior (or the logML if hyperpriors=0), 
% and draws from the posterior distribution of the coefficients and of the covariance matrix of the residuals of the
% GLP-Pandemic Priors BVAR
...
phi = ones(1,covid_periods)*par(end);


%% return a very low value of the posterior if the parameters are outside the bounds
if sum([lambda;psi;theta;miu;alpha;phi(1)]<[MIN.lambda;MIN.psi;MIN.theta;MIN.miu;MIN.alpha;MIN.phi])>0 || ...
        sum([lambda;psi;theta;miu;phi(1)]>[MAX.lambda;MAX.psi;MAX.theta;MAX.miu;MAX.phi])>0
    logML=-10e15;
    betadraw=[];
    drawSIGMA=[];
    return
    ...
    if sur==1
        ...
        xdsur=[1/theta (1/theta)*repmat(y0,1,lags)  zeros(1,covid_periods)];
        ydsur=(1/theta)*y0;
    ...
    if noc==1
        ...
        xdnoc=[zeros(n,1) (1/miu)*repmat(diag(y0),1,lags) zeros(n,covid_periods)];
    ...
    if sur==1 || noc==1
        ...
        if hyperpriors==1
            ...
            logML=logML+logGammapdf(phi(1),priorcoef.phi.k,priorcoef.phi.theta);
        end
    end
    ...
    elseif draw==1
        ...
        cholZZinv = cholred((x'*x+diag(1./omega))\eye(k+covid_periods));
