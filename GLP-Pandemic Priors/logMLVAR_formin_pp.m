function [logML,betahat,sigmahat]=logMLVAR_formin_pp(par,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef,covid_periods)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function computes the log-posterior (or the logML if hyperpriors=0), 
% the posterior mode of the coefficients and the covariance matrix of the residuals of the
...
if mn.alpha==0
    alpha=2;
elseif mn.alpha==1
    alpha=MIN.alpha+(MAX.alpha-MIN.alpha)/(1+exp(-par(end-1)));
end

phi = MIN.phi+(MAX.phi-MIN.phi)/(1+exp(-par(end)));
phi = ones(1,covid_periods)*phi;
