...
println("Working with $n_threads parallel threads")
...
Threads.@threads for iii=1:rps
    if iii ==1
        println("Starting $rps posterior draws with $n_threads threads")
    elseif iii==rps
        println("Posterior draws finished!")
    end
    if test_stab ==0
...
Threads.@threads for iii=1:rps
    if iii ==1
        println("Starting $rps IRFs with $n_threads threads")
    elseif iii==rps
        println("IRFs finished!")
    end
